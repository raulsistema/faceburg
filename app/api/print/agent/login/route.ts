...
export async function POST(request: Request) {
  try {
    ...
    await query(
      `INSERT INTO printer_agents (tenant_id, enabled, agent_key, printer_name, last_seen_at, updated_at)
       VALUES ($1, TRUE, $2, NULLIF($3, ''), NOW(), NOW())
       ON CONFLICT (tenant_id)
       DO UPDATE SET
         enabled = TRUE,
         agent_key = COALESCE(printer_agents.agent_key, EXCLUDED.agent_key),
         printer_name = COALESCE(NULLIF(EXCLUDED.printer_name, ''), printer_agents.printer_name),
         last_seen_at = NOW(),
         updated_at = NOW()`,
      [user.tenant_id, nextKey, printerName],
    );
