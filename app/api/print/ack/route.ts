    ...
    await query(
      `UPDATE print_jobs
       SET status = $3,
           lease_until = NULL,
           last_error = NULLIF($4, ''),
           next_retry_at = CASE WHEN $3 = 'queued' THEN NOW() + INTERVAL '20 seconds' ELSE NULL END,
           updated_at = NOW()
       WHERE id = $1
         AND tenant_id = $2`,
      [jobId, agent.tenant_id, shouldFail ? 'failed' : 'queued', errorMessage || 'Falha de impressao'],
    );
  }
