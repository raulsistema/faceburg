...
import { getPublicMenuProducts, PUBLIC_MENU_CACHE_HEADERS, PUBLIC_MENU_PRODUCT_PAGE_SIZE } from '@/lib/public-menu-data';
...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const result = await getPublicMenuProducts(slug, getProductQuery(request));
    ...
    console.error('[public-menu-products] failed to load products', error);
