...
export async function POST() {
  try {
    ...
    if (!currentSession) {
      return NextResponse.json({ error: 'Entre no sistema antes de ativar o WhatsApp.' }, { status: 401 });
    }
    ...
    await query(
      `INSERT INTO whatsapp_agents
        (tenant_id, enabled, agent_key, session_status, updated_at)
       VALUES
        ($1, FALSE, $2, 'disconnected', NOW())
    ...
    return NextResponse.json({ error: 'Falha ao ativar o agente WhatsApp.' }, { status: 500 });
  }
}
