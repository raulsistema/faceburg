...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (current.type !== 'delivery') {
    return NextResponse.json({ error: 'Somente pedidos de entrega podem ser iniciados pelo motoboy.' }, { status: 400 });
  }
  if (current.status === 'cancelled' || current.status === 'completed') {
    return NextResponse.json({ error: 'Pedido finalizado ou cancelado nao pode sair para entrega.' }, { status: 409 });
  }

  const statusResponse = await forwardStatusChange(request, id, 'delivering');
  const statusData = (await statusResponse.json().catch(() => ({}))) as { error?: string };
  if (!statusResponse.ok) {
    return NextResponse.json(
      { error: statusData.error || 'Falha ao iniciar entrega.' },
