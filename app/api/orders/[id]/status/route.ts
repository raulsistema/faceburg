...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const orderFinance = await client.query<OrderFinanceRow>(
      `SELECT id, total::text, payment_method, payment_method_id
       FROM orders
      ...
      if (paymentMethod === 'card') {
        ...
        if (currentOrder.payment_method_id) {
          const byId = await client.query<PaymentMethodRow>(
            `SELECT id, fee_percent::text, fee_fixed::text, settlement_days
        ...
        if (!selectedMethod) {
          const byType = await client.query<PaymentMethodRow>(
            `SELECT id, fee_percent::text, fee_fixed::text, settlement_days
             FROM payment_methods
             WHERE tenant_id = $1 AND method_type = 'card' AND active = TRUE
          ...
          await client.query(
            `UPDATE orders
             SET payment_method_id = $3,
                 ...
                 payment_net_amount = $5
             WHERE id = $1 AND tenant_id = $2`,
            [id, session.tenantId, selectedMethod.id, feeAmount, netAmount],
          );
