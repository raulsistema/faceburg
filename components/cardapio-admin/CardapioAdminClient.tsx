...
function draftFromProduct(product: Product): ProductDraft {
  ...
  return {
    ...
    printDescription: product.print_description !== false,
    price: product.price,
...
function normalizeDeliveryMaxDistanceKm(value: unknown) {
  const parsed = Number(value ?? 0);
  if (!Number.isFinite(parsed) || parsed <= 0) return 0;
  return Number(Math.min(200, parsed).toFixed(2));
}

function normalizeDeliveryMinOrderAmount(value: unknown) {
  const parsed = parseMoneyInput(value);
  if (!Number.isFinite(parsed) || parsed <= 0) return 0;
  return Number(Math.min(999_999, parsed).toFixed(2));
}
...
function normalizeSettings(settings?: CardapioSettings | null): CardapioSettings {
  return {
    ...
    deliveryMaxDistanceKm: normalizeDeliveryMaxDistanceKm(settings?.deliveryMaxDistanceKm),
    deliveryMinOrderAmount: normalizeDeliveryMinOrderAmount(settings?.deliveryMinOrderAmount),
...
export default function CardapioAdminPage() {
  ...
  async function onSaveProduct(event: FormEvent) {
    ...
    const payload = {
      ...
      printDescription: productDraft.printDescription,
      price: productPrice,
  ...
  function applyDefaultDeliveryFeeTable() {
    const lastDefaultTier = DEFAULT_DELIVERY_FEE_TABLE.at(-1);
    setSettings((current) => ({
      ...current,
      deliveryFeeMode: 'distance_table',
      deliveryFeeTable: DEFAULT_DELIVERY_FEE_TABLE,
      deliveryFeeBase: DEFAULT_DELIVERY_FEE_TABLE[0]?.fee ?? current.deliveryFeeBase,
      deliveryMaxDistanceKm: lastDefaultTier ? Number((lastDefaultTier.upToMeters / 1000).toFixed(2)) : current.deliveryMaxDistanceKm,
    }));
  }
  ...
  function exportProductsCsv() {
    ...
    const header = ['Nome', 'Categoria', 'Tipo', 'SKU', 'Desc. no recibo', 'Status catalogo', 'Disponibilidade', 'Preco'];
    const rows = filteredProducts.map((product) => [
      product.name,
      product.category_name,
      getTypeLabel(product.product_type),
      product.sku || '',
      product.print_description !== false ? 'Sim' : 'Nao',
                        ...
                        >
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {product.description && product.print_description !== false ? (
            ...
            >
              <form onSubmit={onSaveSettings} className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  ...
                  <div>
                    <label className="text-xs font-medium uppercase tracking-[0.16em] text-slate-500">
                      Pedido minimo entrega
                    </label>
                    <div className="mt-2 flex rounded-2xl border border-slate-200 bg-white focus-within:border-orange-300 focus-within:ring-4 focus-within:ring-orange-100">
                      <span className="grid place-items-center rounded-l-2xl border-r border-slate-200 px-4 text-sm font-semibold text-slate-500">
                        R$
                      </span>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
                        value={settings.deliveryMinOrderAmount}
                        onChange={(event) =>
                          setSettings((current) => ({
                            ...current,
                            deliveryMinOrderAmount: normalizeDeliveryMinOrderAmount(event.target.value),
                          }))
                        }
                        className="min-w-0 flex-1 rounded-r-2xl px-4 py-3 text-sm outline-none"
                      />
                    </div>
                    <p className="mt-2 text-xs text-slate-500">
                      {settings.deliveryMinOrderAmount > 0
                        ? `Entrega so avanca com subtotal acima de ${formatCurrency(settings.deliveryMinOrderAmount)}.`
                        : 'Use 0 para liberar qualquer subtotal na entrega.'}
                    </p>
                  </div>
                  <div>
                    <label className="text-xs font-medium uppercase tracking-[0.16em] text-slate-500">
                  ...
                  <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] md:items-start">
                      ...
                      <div className="mt-2 flex rounded-2xl border border-slate-200 bg-white focus-within:border-orange-300 focus-within:ring-4 focus-within:ring-orange-100">
                        <input
                          ...
                          onChange={(event) =>
                            setSettings((current) => ({
                              ...current,
                              deliveryMaxDistanceKm: normalizeDeliveryMaxDistanceKm(event.target.value),
                            }))
                          }
                          className="min-w-0 flex-1 rounded-l-2xl px-4 py-3 text-sm outline-none"
                        />
                        <span className="grid place-items-center rounded-r-2xl border-l border-slate-200 px-4 text-sm font-semibold text-slate-500">
                          km
                        </span>
                      </div>
                    </div>
                    <div className="rounded-2xl bg-white px-4 py-3 text-sm text-slate-600">
                      <span className="block font-semibold text-slate-900">
                        {settings.deliveryMaxDistanceKm > 0
                          ? `Entrega ate ${settings.deliveryMaxDistanceKm.toLocaleString('pt-BR', { maximumFractionDigits: 1 })} km`
                          : 'Sem limite de distancia'}
                      </span>
                      <span className="mt-1 block">
                        Use 0 para nao bloquear. Com limite ativo, o cardapio calcula o trajeto e impede o pedido fora da area.
                      </span>
                    </div>
                  </div>
                </div>
                {settings.deliveryFeeMode === 'distance_table' ? (
                  <div className="rounded-[24px] border border-orange-200 bg-orange-50/60 p-4">
                    ...
                    <div className="mt-3 grid gap-2 text-xs text-slate-600 sm:grid-cols-3">
                      ...
                      <div className="rounded-2xl bg-white px-3 py-2">
                        <span className="block font-semibold text-slate-900">
                          {settings.deliveryMaxDistanceKm > 0 ? 'Acima do limite' : 'Acima da ultima'}
                        </span>
                        {settings.deliveryMaxDistanceKm > 0 ? 'Pedido nao avanca.' : 'Usa a ultima taxa como protecao.'}
                      </div>
                    </div>
                  </div>
                ) : null}
        ...
        {showProductModal ? (
          <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 p-4">
            <div className="mx-auto max-w-5xl rounded-[32px] border border-slate-200 bg-white shadow-[0_24px_90px_-58px_rgba(15,23,42,0.55)]">
              ...
              <div className="grid gap-6 px-6 py-6 xl:grid-cols-[0.88fr_1.12fr]">
                {productModalLoading ? (
                  <div className="grid min-h-[360px] place-items-center rounded-b-[32px] bg-white xl:col-span-2">
                ...
                <form onSubmit={onSaveProduct} className="space-y-4">
                  ...
                  <div>
                    ...
                    <label className="mt-3 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
                      <input
                        type="checkbox"
                        checked={productDraft.printDescription}
                        onChange={(event) =>
                          setProductDraft((current) => ({
                            ...current,
                            printDescription: event.target.checked,
                          }))
                        }
                        className="mt-1 h-4 w-4 rounded border-slate-300 text-orange-500 focus:ring-orange-400"
                      />
                      <span>
                        <span className="block font-semibold text-slate-900">Imprimir descricao no recibo</span>
                        <span className="mt-0.5 block text-xs leading-5 text-slate-500">
                          Deixe ligado nos itens que precisam explicar composicao, sabores ou observacoes fixas no papel.
                        </span>
                      </span>
                    </label>
                  </div>
